...
from WorldModelsExperiments.breakout.rnn.rnn import HyperParams, RNNModel
...
def default_hps():
  return HyperParams(num_steps=4000,
                     max_seq_len=399,  # MIN_LENGTH-1 # train on sequences of 1000 (so 999 + teacher forcing shift)
                     num_actions=4,  # 4 actions
                     encoded_img_width=32,  # width of our data is 32
...
if 'rnn.json' in os.listdir('tf_rnn'):
...
for local_step in range(hps.num_steps):
  ...
  if (step%20==0 and step > 0):
    ...
    train_param.append({
        ...
        'train_step': step.tolist()
    })
  if (step%1000==0 and step>0):
      with open('tf_rnn/rnn_train_param.json','w') as pfile:
