...
class RNNModel():
    def __init__(self, hps, env_name, gpu_mode=True, reuse=False, z_size=32):
        self.hps = hps
    ...
    def build_model(self, hps):
        ...
        INWIDTH = self.z_size + self.num_actions # 36 channels
        OUTWIDTH = self.z_size # 32 channels
...
def rnn_next_state(rnn, z, a, prev_state, env_name):
    if 'Breakout' in env_name:
        input_x = np.concatenate((z.reshape((1, 1, rnn.z_size)), a.reshape((1, 1, 4))), axis=2)
    elif 'CarRacing' in env_name:
        input_x = np.concatenate((z.reshape((1, 1, rnn.z_size)), a.reshape((1, 1, 3))), axis=2)
    feed = {rnn.input_x: input_x, rnn.initial_state: prev_state}
    return rnn.sess.run(rnn.final_state, feed)

def rnn_output_size(rnn):
  return (rnn.z_size+256)
