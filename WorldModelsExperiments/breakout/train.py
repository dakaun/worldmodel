...
### ES related code
#todo reset variables after debugging
#num_episode = 1
...
def initialize_settings(sigma_init=0.1, sigma_decay=0.9999): # todo complete load existing training
...
def worker(weights, seed, train_mode_int=1, max_len=-1):
  ...
  reward_list, t_list, _, _ = simulate(model,
...
def receive_packets_from_slaves():
  ...
  for i in range(1, num_worker+1):
    ...
    for result in results:
      ...
      idx = int(result[1])
      reward_list_total[idx, 0] = result[2] # fitness
      reward_list_total[idx, 1] = result[3] # timesteps
      check_results[idx] = 0
...
def master():
  ...
  while True:
    ...
    # create array to hold fitness results - split on slaves and then get back
    packet_list = encode_solution_packets(seeds, solutions, max_len=max_len)

    send_packets_to_slaves(packet_list)
    reward_list_total = receive_packets_from_slaves()
    # keeps fitness results
    reward_list = reward_list_total[:, 0] # get rewards

    mean_time_step = int(np.mean(reward_list_total[:, 1])*100)/100. # get mean of time step
    max_time_step = int(np.max(reward_list_total[:, 1])*100)/100. # get max time step
    avg_reward = int(np.mean(reward_list)*100)/100. # get mean of reward
    std_reward = int(np.std(reward_list)*100)/100. # get std dev of reward
    # gives list of results back to es
    es.tell(reward_list)
...
if __name__ == "__main__":
  ...
  parser.add_argument('-n', '--num_worker', type=int, default=6)
  ...
  if "parent" == mpi_fork(args.num_worker+1): os.exit() # when nprocs is known shortly after program start, fork nprocs
  main(args)
